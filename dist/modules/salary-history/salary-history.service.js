...
class SalaryHistoryService {
    ...
    static async createSalaryHistory(data) {
        ...
        return salary_history_repository_1.SalaryHistoryRepository.create({
            employeeId: data.employeeId,
    ...
    static async listSalaryHistory(employeeId, currentUserRole, currentUserId, query) {
        ...
        const employee = await salary_history_repository_1.SalaryHistoryRepository.findEmployee(employeeId);
        if (!employee) {
            throw new Error("Employee not found");
        }
    ...
    static async getCurrentSalary(employeeId, currentUserRole, currentUserId) {
        ...
        const salary = await salary_history_repository_1.SalaryHistoryRepository.getCurrentSalary(employeeId);
        return {
            employee,
            salary,
        };
    ...
    static async resolveSalary(employeeId, date, currentUserRole, currentUserId) {
        ...
        const salary = await salary_history_repository_1.SalaryHistoryRepository.resolveSalaryByDate(employeeId, targetDate);
        if (!salary) {
    ...
    static async updateSalaryHistory(id, data, currentUserRole) {
        ...
        return salary_history_repository_1.SalaryHistoryRepository.update(id, {
    ...
    static async deleteSalaryHistory(id, currentUserRole, reason) {
        ...
        return salary_history_repository_1.SalaryHistoryRepository.delete(id);
    }
}
