...
const roundFinalSalary = (amount) => Math.round(amount);
...
const allocateManualAdvanceDeduction = (advances, requestedAmount) => {
    ...
    for (const advance of advances) {
        ...
        const deductedAmount = roundMoney(Math.min(remainingAmount, remainingDeduction));
        if (deductedAmount <= 0)
            continue;
...
const buildPreviewFromPayrollSnapshot = (payroll, employee, periodStart, periodEnd, effectivePeriodStart, effectivePeriodEnd, joinedDuringCycle) => {
    ...
    return {
        ...
        advanceSummary: {
            ...
            advanceDeduction,
            advanceDeductionMode: advanceBreakdown.advanceDeductionMode ?? client_1.AdvanceDeductionMode.AUTO,
...
class SalaryCalculationService {
    static async preview(data, options = {}) {
        const timer = new performanceTimer_1.PerformanceTimer("SalaryCalculationService.preview");
        ...
        const employee = options.employee ??
            (await salary_calculation_repository_1.SalaryCalculationRepository.findEmployee(data.employeeId));
        ...
        const activePayrollSnapshot = options.skipActivePayrollSnapshot || options.recalculationPayrollSnapshot
            ? null
            : await salary_calculation_repository_1.SalaryCalculationRepository.findActivePayrollSnapshot(data.employeeId, periodStart, periodEnd);
        timer.checkpoint("active payroll snapshot fetch");
        if (activePayrollSnapshot) {
            const snapshotPreview = buildPreviewFromPayrollSnapshot(activePayrollSnapshot, employee, periodStart, periodEnd, effectivePeriodStart, effectivePeriodEnd, joinedDuringCycle);
            timer.checkpoint("snapshot preview build");
            timer.end();
            return snapshotPreview;
        }
        const recalculationAdvanceBreakdown = options.recalculationPayrollSnapshot?.advanceBreakdown ?? null;
        const recalculationCarryForwardSummary = recalculationAdvanceBreakdown?.carryForwardApplied ?? null;
        const advanceDeductionMode = employee.advanceDeductionMode ?? client_1.AdvanceDeductionMode.AUTO;
        const [salaryHistories, attendanceRecords, workHourSettings, advances, pendingCarryForwards, unprocessedEarlierAdvances, manualDeduction,] = await Promise.all([
            salary_calculation_repository_1.SalaryCalculationRepository.getSalaryHistories(data.employeeId, effectivePeriodEnd),
            salary_calculation_repository_1.SalaryCalculationRepository.getAttendance(data.employeeId, weeklyCarryInSunday ?? effectivePeriodStart, effectivePeriodEnd),
            overtime_service_1.OvertimeService.getSettingsForDateRange(weeklyCarryInSunday ?? effectivePeriodStart, effectivePeriodEnd),
            recalculationAdvanceBreakdown
                ...
                : advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
                    ? salary_calculation_repository_1.SalaryCalculationRepository.getOutstandingAdvances(data.employeeId, periodEnd)
                    : salary_calculation_repository_1.SalaryCalculationRepository.getAdvancesWithCancelledPayrollSnapshot(data.employeeId, periodStart, periodEnd),
            recalculationCarryForwardSummary
                ? Promise.resolve(recalculationCarryForwardSummary.pendingCarryForwards ?? [])
                : advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
                    ? Promise.resolve([])
                    : payroll_carry_forward_repository_1.PayrollCarryForwardRepository.findPendingByEmployee(data.employeeId, periodStart),
            options.recalculationPayrollSnapshot ||
                advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
                ? Promise.resolve([])
                : salary_calculation_repository_1.SalaryCalculationRepository.getUnprocessedEarlierAdvances(data.employeeId, periodStart),
            advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
                ? options.manualDeductionAmountOverride !== undefined
                    ? Promise.resolve({
                        amount: options.manualDeductionAmountOverride,
                    })
                    : salary_calculation_repository_1.SalaryCalculationRepository.getManualDeduction(data.employeeId, periodStart, periodEnd)
                : Promise.resolve(null),
        ]);
        timer.checkpoint("payroll inputs fetch");
        if (advanceDeductionMode === client_1.AdvanceDeductionMode.AUTO &&
            unprocessedEarlierAdvances.length > 0) {
            const pendingCycles = [
                ...new Set(unprocessedEarlierAdvances.map((advance) => `${formatDate(advance.cycleStartDate)} to ${formatDate(advance.cycleEndDate)}`)),
            ];
            throw new Error(`Earlier advance deduction cycle is not yet processed: ${pendingCycles.join(", ")}. Generate that payroll first so any unpaid balance is carried forward correctly.`);
        }
        ...
        const cycleWorkingDays = Math.max((0, payrollDate_utils_1.getWorkingDatesBetween)(periodStart, periodEnd).length, 1);
        ...
        for (let index = 0; index < salaryTimeline.length; index += 1) {
            ...
            for (const formattedDate of workingDates) {
                ...
                if (dailyOtHours > 0) {
                    ...
                    const workHourSetting = overtime_service_1.OvertimeService.resolveSettingFromList(workHourSettings, workDate);
                    const dailyHours = Math.max(Number(workHourSetting.standardMinutes) / 60, 1);
                    const dailyRate = (Number(salary.salaryAmount) / cycleWorkingDays) / dailyHours;
            ...
            const overtimeSegmentStart = index === 0 && weeklyCarryInSunday
                ? weeklyCarryInSunday
                : segmentStart;
            const sundayOtRecords = attendanceRecords.filter((attendance) => isSunday(attendance.date) &&
                attendance.date >= overtimeSegmentStart &&
                attendance.date <= segmentEnd &&
                Number(attendance.otHours ?? 0) > 0);
            for (const attendance of sundayOtRecords) {
                const dailyOtHours = Number(attendance.otHours ?? 0);
                const workHourSetting = overtime_service_1.OvertimeService.resolveSettingFromList(workHourSettings, attendance.date);
                const dailyHours = Math.max(Number(workHourSetting.standardMinutes) / 60, 1);
                const dailyRate = (Number(salary.salaryAmount) / cycleWorkingDays) /
                    dailyHours;
                const dailyOtEarnings = roundMoney(dailyRate * dailyOtHours);
                otHours = roundMoney(otHours + dailyOtHours);
                otEarnings = roundMoney(otEarnings + dailyOtEarnings);
        ...
        const manualOutstandingTotal = roundMoney(advances.reduce((sum, item) => sum + Number(item.remainingAmount), 0));
        const manualRequestedDeduction = advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
            ? roundMoney(Number(manualDeduction?.amount ?? 0))
            : 0;
        const calculationAdvances = advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
            ? allocateManualAdvanceDeduction(advances, Math.min(manualRequestedDeduction, manualOutstandingTotal))
            : advances;
        const invalidAdvances = calculationAdvances.filter((item) => item.payCycleType !== employee.salaryType);
        if (invalidAdvances.length > 0) {
            throw new Error("Advance pay cycle type does not match employee salary type");
        }
        const hasInvalidRemainingAmount = calculationAdvances.some((item) => Number(item.remainingAmount) < 0);
        if (hasInvalidRemainingAmount) {
            throw new Error("Invalid advance remaining amount found");
        }
        const advanceDeduction = roundMoney(calculationAdvances.reduce((sum, item) => sum +
            Number(advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL
        ...
        if (advanceDeductionMode === client_1.AdvanceDeductionMode.MANUAL &&
            advanceDeduction > grossSalary) {
            throw new Error(`Manual advance deduction cannot exceed payable salary ${grossSalary}. You can deduct up to ${grossSalary} for this cycle.`);
        }
        ...
        for (const item of eligibleCarryForwards) {
            if (remainingGrossForCarryForward <= 0)
                break;
            const applyAmount = roundMoney(Math.min(item.remainingAmount, remainingGrossForCarryForward));
            if (applyAmount <= 0)
                continue;
        ...
        const finalSalary = rawFinalSalary < 0 ? 0 : roundFinalSalary(rawFinalSalary);
        ...
        const result = {
            ...
            advanceSummary: {
                ...
                manualDeductionId: manualDeduction?.id ?? null,
